...
df_profiles = pd.read_csv('persons.csv')
df_profiles.drop(columns=['Unnamed: 0'], inplace=True)
df_profiles_links = pd.read_csv('profiles_links.csv')
df_profiles_links.drop(columns=['Unnamed: 0'], inplace=True)
df_publishers = pd.read_csv('publishers.csv')
df_publishers.drop(columns=['Unnamed: 0'], inplace=True)
df_publishers_links = pd.read_csv('publishers_links.csv')
df_publishers_links.drop(columns=['Unnamed: 0'], inplace=True)
...
async def get_person(soup, url):
    ...
    try:
        ...
        like = None
        print('no like')
    df_profiles.loc[df_profiles.name.str.fullmatch(name), 'desc'] = desc
    df_profiles.loc[df_profiles.name.str.fullmatch(name), 'like'] = like
    df_profiles.loc[df_profiles.name.str.fullmatch(name), 'link_id'] = link_id
    return 0
...
df_profiles['desc'] = 'No desc'
df_profiles['like'] = 'No like'
df_profiles['link_id'] = 'No link'

urls = (df_profiles_links['link'].drop_duplicates()).values
nest_asyncio.apply()
...
df_publishers['link_id'] = df_publishers_links['link'].apply(lambda x: unquote(x.split('/')[-1]))
df_profiles.to_csv('profiles_all_links.csv')
df_publishers.to_csv('publishers_all_links.csv')
